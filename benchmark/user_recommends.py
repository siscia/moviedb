...
def run_evaluation():
    """
    Sets up Django, builds the evaluation dataset, and runs the MLflow evaluation.
    """
    ...
    def predict_fn(user_embedding_context):
        ...
        if isinstance(user_embedding_context, pd.Series):
            ...
            is_batch = True
        elif isinstance(user_embedding_context, list) and len(
            user_embedding_context
        ) > 0 and isinstance(user_embedding_context[0], (list, np.ndarray)):
    ...
    def build_evaluation_dataset(min_interactions=5):
        ...
        users_with_history = (
            UserViewInteraction.objects.values("user_id")
        ...
        for uid in user_ids:
            ...
            inters = (
                UserViewInteraction.objects.filter(
                    user_id=uid, show__embedding__isnull=False
                )
                .select_related("show")
            ...
            for i in train_inters:
                train_data_for_calc.append(
                    {"rating": i.rating, "show": {"embedding": i.show.embedding}}
                )
